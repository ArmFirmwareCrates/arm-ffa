...
impl BootInfo<'_> {
    pub fn pack(descriptors: &[BootInfo], buf: &mut [u8], dest_addr: Option<usize>) {
        ...
        for desc in descriptors {
            ...
            let contents_format = match desc.contents {
                BootInfoContents::Address { content_buf } => {
                    ...
                    // If the buffer containing the boot info blob will be identity mapped, the
                    // total offset we're currently at will be the address of the boot info
                    // contents after copied into the blob. Otherwise the dest_addr argument should
                    // contain the address where the buffer will be mapped.
                    let content_addr = if let Some(dest_addr) = dest_addr {
                        total_offset.checked_add(dest_addr).unwrap()
                    } else {
                        total_offset
                    };
