...
impl PartitionInfo {
    ...
    pub fn pack(descriptors: &[PartitionInfo], buf: &mut [u8], fill_uuid: bool) {
        ...
        for desc in descriptors {
            let mut desc_raw = partition_info_descriptor::default();

            desc_raw.partition_id = desc.partition_id;
            ...
            if fill_uuid {
                desc_raw.uuid.copy_from_slice(&desc.uuid.to_bytes_le());
            }
...
impl Iterator for PartitionInfoIterator<'_> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if self.count > 0 {
            let desc_raw = partition_info_descriptor::ref_from_bytes(
                &self.buf[self.offset..self.offset + PartitionInfo::DESC_SIZE],
            )
            .unwrap();

            let partition_id = desc_raw.partition_id;

            let wrapper = PartPropWrapper::try_from((
                desc_raw.partition_props,
                desc_raw.exec_ctx_count_or_proxy_id,
            ))
            .unwrap();

            let uuid = Uuid::from_bytes_le(desc_raw.uuid);
...
mod tests {
    ...
    fn part_info() {
        ...
        PartitionInfo::pack(&[desc1.clone(), desc2.clone()], &mut buf, true);

        let mut descriptors = PartitionInfoIterator::new(&mut buf, 2);
