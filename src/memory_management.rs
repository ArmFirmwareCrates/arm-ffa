    ...
    fn next(&mut self) -> Option<Self::Item> {
        if self.count > 0 {
            let Ok(desc_raw) = endpoint_memory_access_descriptor::ref_from_bytes(
                &self.buf
                    [self.offset..self.offset + size_of::<endpoint_memory_access_descriptor>()],
            ) else {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if self.count > 0 {
            let Ok(desc_raw) = constituent_memory_region_descriptor::ref_from_bytes(
                &self.buf
                    [self.offset..self.offset + size_of::<constituent_memory_region_descriptor>()],
            ) else {
                return Some(Err(Error::MalformedDescriptor));
            };
...
impl<'a> TryFrom<&[u8]> for MemRelinquishDesc {
    type Error = Error;
    ...
    fn try_from(buf: &[u8]) -> Result<Self, Self::Error> {
        ...
        // The array of endpoint IDs starts at the end of the descriptor. Currently the array most
