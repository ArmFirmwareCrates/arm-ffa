...
impl MemTransactionDesc {
    ...
    pub fn unpack(
        ...
        Error,
    > {
        if buf.len() < size_of::<memory_transaction_descriptor>() {
            return Err(Error::InvalidBufferSize);
        }

        let Ok(transaction_desc_raw) = memory_transaction_descriptor::ref_from_bytes(
            &buf[0..size_of::<memory_transaction_descriptor>()],
        ) else {
        ...
        let Ok(composite_desc_raw) = composite_memory_region_descriptor::ref_from_bytes(
            &buf[offset..offset + size_of::<composite_memory_region_descriptor>()],
        ) else {
            return Err(Error::MalformedDescriptor);
        };
...
impl TryFrom<&[u8]> for MemRelinquishDesc {
    ...
    fn try_from(buf: &[u8]) -> Result<Self, Self::Error> {
        ...
        // this case is supported. The array of endpoint IDs starts at the end of the descriptor and
        // contains only a single u16 element. We can check the descriptor + array size in one step.
        if buf.len() < size_of::<memory_relinquish_descriptor>() + size_of::<u16>() {
            return Err(Error::InvalidBufferSize);
        }

        let Ok(desc_raw) = memory_relinquish_descriptor::ref_from_bytes(
            &buf[0..size_of::<memory_relinquish_descriptor>()],
        ) else {
            return Err(Error::MalformedDescriptor);
        };

        if desc_raw.endpoint_count != 1 {
            return Err(Error::InvalidEndpointCount(desc_raw.endpoint_count));
        }

        let offset = size_of::<memory_relinquish_descriptor>();
        let endpoint = u16::from_le_bytes([buf[offset], buf[offset + 1]]);
