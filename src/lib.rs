...
pub enum Error {
    ...
    InvalidRegisterNumVersion(u8, Version),
    #[error("Invalid number of registers ({0}) for function {1}")]
    InvalidRegisterNumFunction(u8, u32),
}
...
/// Register contents used for SVC/HVC/SMC and ERET conduits.
pub enum SyscallRegs<'a> {
    Regs8(&'a mut [u64; 8]),
    Regs18(&'a mut [u64; 18]),
}

/// FF-A interface register contents and the version used at the FF-A instance.
pub struct FfaRegs<'a> {
    pub version: Version,
    pub regs: SyscallRegs<'a>,
}
...
impl TryFrom<FfaRegs<'_>> for Interface {
    type Error = Error;

    fn try_from(value: FfaRegs<'_>) -> Result<Self, Error> {
        let msg = match value.regs {
            SyscallRegs::Regs8(regs) => {
                if value.version > Version(1, 1) {
                    return Err(Error::InvalidRegisterNumVersion(8, value.version));
                }

                Interface::unpack_regs8(value.version, regs)?
            }
            SyscallRegs::Regs18(regs) => {
                if value.version < Version(1, 2) {
                    return Err(Error::InvalidRegisterNumVersion(18, value.version));
                }

                match FuncId::try_from(regs[0] as u32)? {
                    FuncId::ConsoleLog64 | FuncId::Success64 => {
                        Interface::unpack_regs18(value.version, regs)?
                    }
                    _ => {
                        let [ref regs8 @ .., _, _, _, _, _, _, _, _, _, _] = regs;
                        Interface::unpack_regs8(value.version, regs8)?
                    }
                }
            }
        };
...
impl Interface {
    ...
    pub fn to_regs(&self, value: FfaRegs) {
        match value.regs {
            SyscallRegs::Regs8(regs) => {
                assert!(value.version <= Version(1, 1));
                self.pack_regs8(value.version, regs);
            }
            SyscallRegs::Regs18(regs) => {
                assert!(value.version >= Version(1, 2));
                ...
                match self {
                    ...
                    | Interface::MsgSendDirectResp2 { .. } => {
                        self.pack_regs18(value.version, regs);
                    }
                    _ => {
                        let [ref mut regs8 @ .., _, _, _, _, _, _, _, _, _, _] = regs;
                        self.pack_regs8(value.version, regs8);
                    }
                }
            }
        }
