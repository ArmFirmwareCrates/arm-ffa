...
impl Interface {
    ...
    pub fn function_id(&self) -> Option<FuncId> {
        match self {
            ...
            Interface::MemPermGet { buf, .. } => match buf {
                MemOpBuf::Buf32 { .. } => Some(FuncId::MemPermGet32),
                MemOpBuf::Buf64 { .. } => Some(FuncId::MemPermGet64),
            },
            Interface::MemPermSet { buf, .. } => match buf {
                MemOpBuf::Buf32 { .. } => Some(FuncId::MemPermSet32),
                MemOpBuf::Buf64 { .. } => Some(FuncId::MemPermSet64),
            },
    ...
    pub fn is_32bit(&self) -> bool {
        match self {
            ...
            Interface::MemPermGet { buf, .. } | Interface::MemPermSet { buf, .. }
                if matches!(buf, MemOpBuf::Buf32 { .. }) =>
            {
                true
            }
    ...
    pub fn copy_to_array(&self, a: &mut [u64; 8]) {
        ...
        match *self {
            ...
            Interface::MemPermGet { buf } => {
                a[1] = match buf {
                    MemOpBuf::Buf32 { addr, .. } => addr.into(),
                    MemOpBuf::Buf64 { addr, .. } => addr,
                };
            }
            Interface::MemPermSet { buf, mem_perm } => {
                (a[1], a[2]) = match buf {
                    MemOpBuf::Buf32 { addr, page_cnt } => (addr.into(), page_cnt.into()),
                    MemOpBuf::Buf64 { addr, page_cnt } => (addr, page_cnt.into()),
                };
                a[3] = mem_perm.into();
            }
            Interface::ConsoleLog {
