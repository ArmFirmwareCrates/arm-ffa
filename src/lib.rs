    ...
    fn try_from(regs: [u64; 8]) -> Result<Self, Error> {
        ...
        let msg = match fid {
            ...
            FuncId::MemDonate32 => Self::MemDonate {
                ...
                addr: MemOpAddr::Addr32(regs[3] as u32),
                page_cnt: regs[4] as u32,
            },
            FuncId::MemDonate64 => Self::MemDonate {
                total_len: regs[1] as u32,
                frag_len: regs[2] as u32,
                addr: MemOpAddr::Addr64(regs[3]),
                page_cnt: regs[4] as u32,
            },
            ...
            FuncId::MemPermGet32 => Self::MemPermGet {
                addr: MemOpAddr::Addr32(regs[1] as u32),
            },
            FuncId::MemPermGet64 => Self::MemPermGet {
                addr: MemOpAddr::Addr64(regs[1]),
            },
...
impl Interface {
    ...
    pub fn function_id(&self) -> Option<FuncId> {
        match self {
            ...
            Interface::MemDonate { addr, .. } => match addr {
                MemOpAddr::Addr32(_) => Some(FuncId::MemDonate32),
                MemOpAddr::Addr64(_) => Some(FuncId::MemDonate64),
            },
            Interface::MemLend { addr, .. } => match addr {
                MemOpAddr::Addr32(_) => Some(FuncId::MemLend32),
                MemOpAddr::Addr64(_) => Some(FuncId::MemLend64),
            },
            Interface::MemShare { addr, .. } => match addr {
                MemOpAddr::Addr32(_) => Some(FuncId::MemShare32),
                MemOpAddr::Addr64(_) => Some(FuncId::MemShare64),
            },
            Interface::MemRetrieveReq { addr, .. } => match addr {
                MemOpAddr::Addr32(_) => Some(FuncId::MemRetrieveReq32),
                MemOpAddr::Addr64(_) => Some(FuncId::MemRetrieveReq64),
            },
            ...
            Interface::MemPermGet {
                ...
                MemOpAddr::Addr32(_) => Some(FuncId::MemPermGet32),
                MemOpAddr::Addr64(_) => Some(FuncId::MemPermGet64),
            },
            Interface::MemPermSet {
                addr: base_addr, ..
            } => match base_addr {
                MemOpAddr::Addr32(_) => Some(FuncId::MemPermSet32),
                MemOpAddr::Addr64(_) => Some(FuncId::MemPermSet64),
            },
    ...
    pub fn is_32bit(&self) -> bool {
        match self {
            ...
            Interface::MemDonate { addr, .. }
            | Interface::MemLend { addr, .. }
            | Interface::MemShare { addr, .. }
            | Interface::MemRetrieveReq { addr, .. }
            | Interface::MemPermGet { addr, .. }
            | Interface::MemPermSet { addr, .. }
                if matches!(addr, MemOpAddr::Addr32 { .. }) =>
            {
                true
            }
    ...
    pub fn copy_to_array(&self, a: &mut [u64; 8]) {
        ...
        match *self {
            Interface::Error {
                ...
                a[1] = u32::from(target_info) as u64;
            ...
            Interface::Version { input_version } => {
                a[1] = u32::from(input_version) as u64;
            }
            Interface::VersionOut { output_version } => {
                a[0] = u32::from(output_version) as u64;
            }
            Interface::Features {
                feat_id,
                input_properties,
            } => {
                a[1] = u32::from(feat_id) as u64;
            ...
            Interface::MemDonate {
                ...
                a[2] = frag_len as u64;
                a[3] = match addr {
                    MemOpAddr::Addr32(addr) => addr as u64,
                    MemOpAddr::Addr64(addr) => addr,
                };
                a[4] = page_cnt as u64;
            }
            Interface::MemLend {
