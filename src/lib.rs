...
pub enum Error {
    ...
    #[error("Unrecognised FF-A VM SP Return Status {0}")]
    UnrecognisedVMSpReturnStatus(i32),
    #[error("Unrecognised PSCI Return Status {0}")]
    UnrecognisedPsciReturnStatus(i32),
...
impl From<Error> for FfaError {
    fn from(value: Error) -> Self {
        match value {
            ...
            | Error::UnrecognisedVMSpReturnStatus(_)
            | Error::UnrecognisedPsciReturnStatus(_)
...
#[derive(Clone, Copy, Debug, Eq, IntoPrimitive, PartialEq, TryFromPrimitive)]
#[num_enum(error_type(name = Error, constructor = Error::UnrecognisedVMSpReturnStatus))]
#[repr(i32)]
pub enum VMSpReturnStatus {
    // The SP acknowledges successful receipt of VM creation message by transitioning to the VM
    // available state.
    Success = 0,
    // One or more parameters were incorrectly encoded. One or more parameters contain invalid
    // values. The SP transitions to the Error state.
    InvalidParameters = -2,
    // The SP was interrupted by a Non-secure interrupt. It performed a managed exit before handling
    // the message. The Hypervisor should resend the message to resume SP execution. This enables
    // the SP to finish handling the VM creation message. The SP remains in the VM unavailable state
    Interrupted = -5,
    // The SP cannot acknowledge successful receipt of VM creation message due to an IMPLEMENTATION
    // DEFINED reason. The SP remains in its current state.
    Denied = -6,
    // The SP is in an IMPLEMENTATION DEFINED state that prevents it from acknowledging the VM
    // creation message. The Hypervisor should resend the VM creation message. The SP remains in the
    // VM unavailable state.
    Retry = -7,
}

#[derive(Clone, Copy, Debug, Eq, IntoPrimitive, PartialEq, TryFromPrimitive)]
#[num_enum(error_type(name = Error, constructor = Error::UnrecognisedPsciReturnStatus))]
#[repr(i32)]
pub enum PsciReturnStatus {
    Success = 0,
    Denied = -3,
}
...
pub enum DirectMsgArgs {
    ...
    // * Encoding of w0 in Table 13.5 if a version is returned by the SPMC.
    // * NOT_SUPPORTED if a version is not returned by the SPMC. (None)
    VersionResp {
    ...
    PowerPsciReq32 {
        function_id: u32,
        // params[i]: Input parameter in wi in PSCI function invocation at EL3.
        params: [u32; 3],
    },
    // Message for a power management operation initiated by a PSCI function
    PowerPsciReq64 {
        function_id: u32,
        // params[i]: Input parameter in xi in PSCI function invocation at EL3.
        params: [u64; 3],
    },
    ...
    // Return error code SUCCESS or DENIED as defined in PSCI spec
    PowerPsciResp {
        psci_status: PsciReturnStatus,
    },
    ...
    CreateVmReq {
        ...
        //  used.
        handle: u64,
        // ID of VM that has been created. (Bit[31:16]: Reserved (SBZ))
        vm_id: u16,
    },
    // Message to acknowledge creation of a VM
    AckCreateVmResp {
        sp_status: VMSpReturnStatus,
    },
...
impl DirectMsgArgs {
    ...
    // Bit[31] = b'1: Framework message
    const FWK_MSG_BITS: u32 = 1 << 31;
    // Bit[30:8] = 0: Reserved (SBZ); Bit[7:0] = b’00001000
    const VERSION_REQ: u32 = DirectMsgArgs::FWK_MSG_BITS | 0b1000;
    // Bit[30:8] = 0: Reserved (SBZ); Bit[7:0] = b’00001001
    const VERSION_RESP: u32 = DirectMsgArgs::FWK_MSG_BITS | 0b1001;
    // Bit[30:8] = 0: Reserved (SBZ); Bit[7:0] = b’00000000
    const POWER_PSCI_REQ32: u32 = DirectMsgArgs::FWK_MSG_BITS;
    const POWER_PSCI_REQ64: u64 = DirectMsgArgs::POWER_PSCI_REQ32 as u64;
    // Bit[30:8] = 0: Reserved (SBZ); Bit[0]: Warm boot type
    // b’0: Exit from a suspend to RAM state, b’1: Exit from a low power state shallower than the
    // suspend to RAM state
    const POWER_WARM_BOOT_REQ: u32 = DirectMsgArgs::FWK_MSG_BITS | 0b0001;
    // Bit[30:8] = 0: Reserved (SBZ); Bit[7:0] = b’00000010
    const POWER_PSCI_RESP: u32 = DirectMsgArgs::FWK_MSG_BITS | 0b0010;
    // Bit[30:8] = 0: Reserved (SBZ); Bit[7:0] = b’00000100
    const CREATE_VM_REQ: u32 = DirectMsgArgs::FWK_MSG_BITS | 0b0100;
    // Bit[30:8] = 0: Reserved (SBZ); Bit[7:0] = b’00000101
    const ACK_CREATE_VM_RESP: u32 = DirectMsgArgs::FWK_MSG_BITS | 0b0101;
    // Bit[30:8] = 0: Reserved (SBZ); Bit[7:0] = b’00000110
    const DESTRUCT_VM_REQ: u32 = DirectMsgArgs::FWK_MSG_BITS | 0b0110;
    // Bit[30:8] = 0: Reserved (SBZ); Bit[7:0] = b’00000111
    ...
    pub fn function_id(&self) -> Option<FuncId> {
        match self {
            ...
            Interface::MsgSendDirectReq { args, .. } => match args {
                ...
                DirectMsgArgs::CreateVmReq { .. } => Some(FuncId::MsgSendDirectReq32),
                DirectMsgArgs::DestructVmReq { .. } => Some(FuncId::MsgSendDirectReq32),
                _ => None,
            },
            Interface::MsgSendDirectResp { args, .. } => match args {
                ...
                DirectMsgArgs::AckCreateVmResp { .. } => Some(FuncId::MsgSendDirectResp32),
                DirectMsgArgs::AckDestructVmResp { .. } => Some(FuncId::MsgSendDirectResp32),
                _ => None,
            },
        ...
        let msg = match fid {
            ...
            FuncId::MsgSendDirectReq32 => Self::MsgSendDirectReq {
                ...
                args: if (regs[2] as u32 & DirectMsgArgs::FWK_MSG_BITS) != 0 {
                    match regs[2] as u32 {
                        ...
                        DirectMsgArgs::POWER_PSCI_REQ32 => DirectMsgArgs::PowerPsciReq32 {
                        ...
                        DirectMsgArgs::CREATE_VM_REQ => DirectMsgArgs::CreateVmReq {
                            handle: (regs[3] << 32) | regs[4],
                            vm_id: regs[5] as u16,
                        },
                        DirectMsgArgs::DESTRUCT_VM_REQ => DirectMsgArgs::DestructVmReq {
                            handle: (regs[3] << 32) | regs[4],
                            vm_id: regs[5] as u16,
                        },
            ...
            FuncId::MsgSendDirectReq64 => Self::MsgSendDirectReq {
                ...
                args: if (regs[2] & DirectMsgArgs::FWK_MSG_BITS as u64) != 0 {
                    match regs[2] {
                        DirectMsgArgs::POWER_PSCI_REQ64 => DirectMsgArgs::PowerPsciReq64 {
                            function_id: regs[3] as u32,
                ...
                args: if (regs[2] as u32 & DirectMsgArgs::FWK_MSG_BITS) != 0 {
                    match regs[2] as u32 {
                        ...
                        DirectMsgArgs::POWER_PSCI_RESP => DirectMsgArgs::PowerPsciResp {
                            psci_status: (regs[3] as i32).try_into()?,
                        },
                        DirectMsgArgs::ACK_CREATE_VM_RESP => DirectMsgArgs::AckCreateVmResp {
                            sp_status: (regs[3] as i32).try_into()?,
                        },
                        DirectMsgArgs::ACK_DESTRUCT_VM_RESP => DirectMsgArgs::AckDestructVmResp {
                            sp_status: (regs[3] as i32).try_into()?,
                        },
                        _ => return Err(Error::UnrecognisedFwkMsg(regs[2] as u32)),
                    }
            ...
            Interface::MsgSendDirectReq {
                ...
                match args {
                    ...
                    DirectMsgArgs::VersionReq { version } => {
                        a[2] = DirectMsgArgs::VERSION_REQ as u64;
                        a[3] = <Version as Into<u32>>::into(version) as u64;
                    }
                    DirectMsgArgs::PowerPsciReq32 {
                        function_id,
                        params,
                    } => {
                        a[2] = DirectMsgArgs::POWER_PSCI_REQ32 as u64;
                        a[3] = function_id as u64;
                        a[4] = params[0] as u64;
                        a[5] = params[1] as u64;
                        a[6] = params[2] as u64;
                    }
                    DirectMsgArgs::PowerPsciReq64 {
                        ...
                        a[2] = DirectMsgArgs::POWER_PSCI_REQ64;
                        a[3] = function_id as u64;
                        a[4] = params[0];
                        a[5] = params[1];
                        a[6] = params[2];
                    ...
                    DirectMsgArgs::PowerWarmBootReq { boot_type } => {
                        a[2] = DirectMsgArgs::POWER_WARM_BOOT_REQ as u64;
                        a[3] = boot_type as u64;
                    }
                    DirectMsgArgs::CreateVmReq { handle, vm_id } => {
                        a[2] = DirectMsgArgs::CREATE_VM_REQ as u64;
                        a[3] = handle & 0xffff_0000;
                        a[4] = handle & 0x0000_ffff;
                        a[5] = vm_id as u64;
                    }
                    DirectMsgArgs::DestructVmReq { handle, vm_id } => {
                        a[2] = DirectMsgArgs::DESTRUCT_VM_REQ as u64;
                        a[3] = handle & 0xffff_0000;
                        a[4] = handle & 0x0000_ffff;
                        a[5] = vm_id as u64;
                    }
            ...
            Interface::MsgSendDirectResp {
                ...
                match args {
                    ...
                    DirectMsgArgs::VersionResp { version } => {
                        a[2] = DirectMsgArgs::VERSION_RESP as u64;
                        match version {
                            None => {
                                a[3] = <FfaError as Into<i32>>::into(FfaError::NotSupported) as u64
                            }
                            Some(ver) => a[3] = <Version as Into<u32>>::into(ver) as u64,
                        }
                    }
                    DirectMsgArgs::PowerPsciResp { psci_status } => {
                        a[2] = DirectMsgArgs::POWER_PSCI_RESP as u64;
                        a[3] = <PsciReturnStatus as Into<i32>>::into(psci_status) as u64;
                    }
                    DirectMsgArgs::AckCreateVmResp { sp_status } => {
                        a[2] = DirectMsgArgs::ACK_CREATE_VM_RESP as u64;
                        a[4] = <VMSpReturnStatus as Into<i32>>::into(sp_status) as u64;
                    }
                    DirectMsgArgs::AckDestructVmResp { sp_status } => {
                        a[2] = DirectMsgArgs::ACK_DESTRUCT_VM_RESP as u64;
                        a[3] = <VMSpReturnStatus as Into<i32>>::into(sp_status) as u64;
                    }
